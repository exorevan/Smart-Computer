...
class DoubleFileCrypt(CryptHandler):
    ...
    def _custom_function(self, crypt: bool) -> None:
        ...
        addit_lines_array: list[str] = []
        processed_lines_array: list[str] = []

        line: str
        char: str
        addit_new_char_int: int
        ...
        additional_file = open(file=self.additional_file, mode="r")

        orig_line: str
        addit_line: str
        for orig_line, addit_line in zip(
            orig_file.readlines(), additional_file.readlines()
        ):
